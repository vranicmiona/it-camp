...
// Postoji u JavaScriptu bulit-in metoda koja proverava da li je neki broj ceo Intager:
// Number.isIntager()
...
function lookUpProfile(arr, name, prop) {
  const element = arr.find((el) => {
    ...
    el.firstName === name && keys.includes(prop);
  });
  if (element) {
    return element[prop];
  } else {
...
console.log(lookUpProfile(contacts, "Akira", "number"));
